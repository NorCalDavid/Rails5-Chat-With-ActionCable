class Notification < ApplicationRecord
  ...
  def stream_notification
    ...
    if self.notifiable.nil?
      ...
      message = "#{slef.actor.name} #{self.action} you #{time_ago_in_words(self.created_at)}"
      ActionCable.server.broadcast "notifications:#{self.recipient_id}", html: html, count: count, color: "red", message: message
    else
